...
fn parse(input: &str) -> ParseResult {
    let hot_tub_situations =  input.lines()
        .map(|line| {
            ...
            let mut conditions = condition_str.chars()
                .map(|c| match c {
                ...
                .collect::<Vec<_>>();
            let mut damaged_group_sizes = damaged_str.split(',')
                .map(|damaged_num_str| {
                    damaged_num_str.parse::<u8>().expect(&format!("failed to parse into number: {damaged_num_str}"))
                })
                .collect::<Vec<_>>();
            // TODO gotta be a cleaner way to repeat + join conditions
            conditions.push(Condition::Unknown);
            conditions = (0..5).flat_map(|_| conditions.clone())
                .collect::<Vec<_>>();
            conditions.pop();
            damaged_group_sizes = damaged_group_sizes.repeat(5);
