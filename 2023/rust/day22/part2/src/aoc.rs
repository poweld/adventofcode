mod my {
    ...
    impl Tower {
        ...
        pub fn disintegration_damage(&self) -> u64 {
            let mut damage: u64 = 0;
            for brick in 0..self.bricks_data.len() {
                let mut to_destroy = VecDeque::from([brick]);
                let mut will_fall: HashSet<Brick> = HashSet::from([brick]);
                while to_destroy.len() > 0 {
                    let brick = to_destroy.pop_front().unwrap();
                    for supported_by in self.get_supported_by(&brick) {
                        let supporting = self.get_supporting(&supported_by);
                        if supporting.iter().all(|supporting_brick| will_fall.contains(supporting_brick)) {
                            will_fall.insert(supported_by);
                            to_destroy.push_back(supported_by);
                        }
                    }
                }
                damage += (will_fall.len() as u64) - 1;
            }
            damage
        }
...
pub fn solve(input_path: &str) -> String {
    ...
    tower.disintegration_damage().to_string()
}
